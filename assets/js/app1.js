...
var main_ul = document.querySelector(".add_tasks");
...
add_task.onclick = function () {
  ...
  if (input_text.value.length > 5) {
    ...
    var list_items = `<hr class="mt-2"> 
      <div class= "my-4 flex justify-between px-1">
      <div class="flex items-center gap-2">
        <span onclick = "tick(${i})" class ="h-8 cursor-pointer w-8 flex justify-center items-center border border-gray-600 rounded-full">
        <i id = "checked${i}" class="transition-all hover:text-lg text-blue-500 hover:text-blue-900 fa fa-check"></i></span>
        <p class="md:max-w-[375px] truncate max-w-[280px] ">
         <strike id="strike${i}" class="text-gray-600 text-sm no-underline">${input_text.value}</strike>
        </p>
    </div>
    <span onclick="trash(${i})" class="h-8 cursor-pointer w-8 flex justify-center items-center border border-gray-600 rounded-full ">
...
function trash(random_id) {
  ...
  main_ul.removeAttribute(get_id);
}
